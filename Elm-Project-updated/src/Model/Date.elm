...
monthsBetween dA dB =
    let
        monthsInAYear = 12
        monthsA = Maybe.withDefault 0 (Maybe.map monthToInt (month dA))
        monthsB = Maybe.withDefault 0 (Maybe.map monthToInt (month dB))
        yearsDiff = abs (year dA - year dB)
        answer =   
         if year dA >= year dB then
                Just (abs((year dA - year dB) * monthsInAYear + (monthsA - monthsB)))
         else
                Just (abs((year dB - year dA) * monthsInAYear  - (monthsA - monthsB)))
...
compare (Date d1) (Date d2) =
               let
                ...
                in  chainCompare equalYear (Basics.compare d1.year d2.year)
...
view (Date d) =
     div []
        [ text <| createDateView (Date d) ]

createDateView : Date -> String
createDateView (Date date) =
    case date.month of
        Just m ->
            monthToString m ++ " " ++ String.fromInt date.year
